...
/*                                         ***                                         */
/*    Only professionel OS like OS/2 or eComStation contains all the stuff you need    */
/* *********************************************************************************** */
...
FileList       = 'GetTypo3forOS2List.txt'

/* some variables */
LogFile        = 'GetTypo3forOS2-'||Date(S)||'-'||Time(H)||Time(M)//60||'.log'
...
strTypo3_OS_01  = "define('TYPO3_OS', (stristr(PHP_OS,'win')&&!stristr(PHP_OS,'darwin'))||stristr(PHP_OS,'os/2')?'WIN':''); // changed by Team Typo3-goes-eComStation"
...
IF rsdebug = "1" THEN SAY 'Press any key to continue ...'
IF rsdebug = "1" THEN Taste = SysGetKey(NoEcho)
IF s.0 \= 0 THEN CALL LineOut Logfile, Date(E)||'-'||Time()||' ... Typo3-package-folder exist: '||t3folder||' -> continue with filecheck'
IF s.0 = 0 THEN CALL CharOut Logfile, Date(E)||'-'||Time()||' ... Typo3-package-folder does not exist: '||t3folder||' -> continue with archive-check'
IF s.0 = 0 THEN
   DO
         ...
         IF STREAM(tool_file_03, 'c', 'query exists') = '' THEN
            DO
               ...
               ELSE
                  ...
                  END
            END
         /* wget the desired typo3-package */
               ...
               DO
                  ...
                  IF POS(TRANSLATE(strTypo3_OS_00), TRANSLATE(SourceLine)) > '0' THEN
                     DO
                        ...
                        SourceLine = INSERT(strTypo3_OS_01, SourceLine, ChangePos-1, LENGTH(strTypo3_OS_01))
...
HelpProgram:
        ...
        SAY '-dl:1       download from Reston (North America)'
        SAY '-dl:2       download from Curitiba (South America'
        SAY '-dl:3       download from Ishikawa (Asia)'
        SAY '-dl:4       download from Sydney (Australia)'
        SAY '-dl:5       download from Paris (Europe)          (default)'
        SAY '-t3:1       download Typo3-quickstart-3.8.0       (default)'
        SAY '-t3:2       download Typo3-dummy-3.8.0'
        SAY '-t3:3       download Typo3-testsite-3.8.0'
        SAY '-t3:4       download Typo3-quickstart-3.8.1'
        SAY '-t3:5       download Typo3-dummy-3.8.1'
        SAY '-t3:6       download Typo3-testsite-3.8.1'
...
ShowResults:
      ...
      IF ChangedCount = FoundCount THEN
         DO
            SAY "           _____                 _____                               "
            SAY "          |_   _|   _ _ __   ___|___ /    __ _  ___   ___  ___       "
            SAY "            | || | | | '_ \ / _ \ |_ \   / _` |/ _ \ / _ \/ __|      "
            SAY "            | || |_| | |_) | (_) |__) | | (_| | (_) |  __/\__ \      "
            SAY "            |_| \__, | .__/ \___/____/   \__, |\___/ \___||___/      "
            SAY "                |___/|_|                 |___/                       "
            SAY "             ____                ____  _        _   _                "
            ...
            SAY
            SAY '                   Paul Smedley, paul@smedley.info'
            SAY '                 Michael Oehlhof, michael@oehlhof.de'
            SAY '          Roland Schmalenberg, roland.schmalenberg@lycos.de'
         END
      ELSE
         DO
